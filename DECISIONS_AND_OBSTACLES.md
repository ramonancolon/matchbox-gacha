...
- **Contributor setup friction**: Requiring every new developer to immediately configure production Firebase and deploy Functions created onboarding drag and avoidable CORS/App Check confusion. The default local flow was changed so `npm run dev` now starts emulators (`dev:local`) and routes callables locally; cloud-backed development remains available via `npm run dev:cloud`.
